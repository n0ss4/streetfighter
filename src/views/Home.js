 export default {
   ...
   methods: {
    ...
    atacar: function (e){
      ...
      if(e == this.nombre){
        this.message = this.nombre+" ataca a "+this.enemigo[this.numeroEnemigo];
        // Prueba de ataque
        this.vidaEnemigo = this.vidaEnemigo - (Math.floor(Math.random() * 5)+8);
        this.turno = 1;
      }
      
      if(e == this.enemigo[this.numeroEnemigo]){
          this.message = this.enemigo[this.numeroEnemigo]+" ataca a "+this.nombre;
          // Prueba de ataque
    ...
    curar: function(e){
      ...
      if(e == this.enemigo[this.numeroEnemigo]){
          ...
          // Prueba de curación
          this.vidaEnemigo = this.vidaEnemigo + (Math.floor(Math.random() * 14)+6);
    ...
    especial: function(e){
      ...
      if(e == this.nombre){
          this.message = this.nombre+" va a usar su ataque especial cuidado "+this.enemigo[this.numeroEnemigo];
          this.turno = 1;
          this.vidaEnemigo = this.vidaEnemigo - Math.floor(Math.random() * 31);
      }
      
      if(e == this.enemigo[this.numeroEnemigo]){
          this.message = this.enemigo[this.numeroEnemigo]+" va a usar su ataque especial cuidado "+this.nombre;
          this.turno = 0;
    ...
    // RENDIR-SE.
    rendir: function(){
        this.message = this.nombre+" ya no puede más y se va a retirar... gana "+this.enemigo[this.numeroEnemigo];
        // Prueba de rendirse
        this.vida = 0;
        this.turno = 1;
    ...
    cpu: function(){
        ...
        setTimeout(function(){
            switch(Math.floor(Math.random() * 3)){
                case 0: self.atacar(self.enemigo[self.numeroEnemigo]); break;
                case 1: self.curar(self.enemigo[self.numeroEnemigo]); break;
                case 2: self.especial(self.enemigo[self.numeroEnemigo]); break;
            }
        }, 500);
